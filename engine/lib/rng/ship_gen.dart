...
class ShipGenerator {
  static Ship generateShip(System system, Galaxy galaxy, Random rnd, {isPirate = false}) {
    ...
    final pilot = Pilot(Rng.generateName(rnd: rnd),rnd: rnd, sector: location, galaxy: galaxy, isPirate: isPirate);
    final level = max(0,1 - (galaxy.topo.distance(location.system, galaxy.findHomeworld(pilot.faction.species)) / galaxy.maxJumps));
    ...
    Ship ship = Ship("HMS ${Rng.randomAlienName(rnd)}",pilot: pilot, shipClass: ShipClass.fromEnum(shipClassType));
